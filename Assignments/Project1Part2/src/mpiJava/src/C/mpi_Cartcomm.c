  ...
  (JNIEnv *env, jobject jthis)
{
  ...
  MPI_Cartdim_get((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),&maxdims);
  ...
  MPI_Cart_get((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
...
JNIEXPORT jobject JNICALL Java_mpi_Cartcomm_Shift
  (JNIEnv *env, jobject jthis, jint direction, jint disp)
{
  int sr, dr;
  ...
  MPI_Cart_shift((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)), 
...
*/
  ...
  clearFreeList(env) ;
    ...
    MPI_Cartdim_get((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
		&maxdims);
	jcoords=(*env)->NewIntArray(env,maxdims);
	coords=(*env)->GetIntArrayElements(env,jcoords,&isCopy);
	MPI_Cart_coords((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
...
JNIEXPORT jint JNICALL Java_mpi_Cartcomm_Map
  (JNIEnv *env, jobject jthis, jintArray dims, jbooleanArray periods)
{
    int newrank;
    jint *ds;
    jboolean *ps;
    jboolean isCopy=JNI_TRUE;
    int ndims;
	int *int_re_ds=(int*)calloc(
		 (*env)->GetArrayLength(env,periods), sizeof(int));
	int i;
  ...
  clearFreeList(env) ;
    ...
    MPI_Cart_map((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
...
JNIEXPORT jint JNICALL Java_mpi_Cartcomm_Rank
  (JNIEnv *env, jobject jthis, jintArray coords)
{
	int rank;
	jint *crds;
	jboolean isCopy=JNI_TRUE;
  ...
  clearFreeList(env) ;
	...
	MPI_Cart_rank((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
        (int*)crds, &rank);
...
JNIEXPORT jlong JNICALL Java_mpi_Cartcomm_sub
  (JNIEnv *env, jobject jthis, jbooleanArray remain_dims)
{
	MPI_Comm newcomm;
	jboolean *re_ds;
	jboolean isCopy=JNI_TRUE;
	int *int_re_ds=(int*)calloc(
		 (*env)->GetArrayLength(env,remain_dims), sizeof(int));
	int i;
  ...
  clearFreeList(env) ;
	...
	MPI_Cart_sub((MPI_Comm)((*env)->GetLongField(env,jthis,CommhandleID)),
		     int_re_ds, &newcomm);
