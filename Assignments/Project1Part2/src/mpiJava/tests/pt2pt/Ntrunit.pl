...
#
# A quick and dirty perl-script to compile and run pt2pt test codes
#

  @files = ('bsend', 'buffer', 'getcount', 'interf', 'isend', 'probe',
            'rsend', 'rsend2', 'sendrecv', 'seq', 'ssend', 'start', 'startall', 
            'test1', 'test2', 'test3', 'testall', 'testany', 'testsome', 'waitall', 
            'waitany', 'waitsome', 'wildcard');
...
#
   $command_line = "$filename.exe -p4gm 40000000 -p4pg $filename.pg";
...
#
  ...
  print "\nCompleted tests - cleaning up pt2pt directory \n";
