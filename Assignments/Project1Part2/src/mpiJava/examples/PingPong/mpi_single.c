...
int main(int argc, char * argv[])
{
    ...
    int        n_init, n_mess;
    double     *A;
    /* char       *Title ="Single Messages --- MPI_C on Solaris(SM)"; */
    char       *Title ="Single Messages --- MPI_C";
    FILE       *ifp;
    ...
    if (my_pe == 0) {
       ifp = fopen("mpi_single_C_SM2.dat","w");
    ...
    for (log2nbyte = 0; log2nbyte <= LOG2N_MAX; ++log2nbyte) {
	   ...
	   for (j_pe = 1; j_pe <= npes-1; ++j_pe) {
		  ...
		  /*printf("my PE = %d \n", my_pe);*/
                ...
                if (my_pe == 0) {
		  /*printf("my PE = %d \n", my_pe);*/
                   MPI_Recv(A,nbyte,MPI_BYTE,j_pe,10,MPI_COMM_WORLD,&status);
                   MPI_Send(A,nbyte,MPI_BYTE,j_pe,20,MPI_COMM_WORLD);
                }
             }
          tf[ns+1] = MPI_Wtime();
          }
